...
class InvalidateUserCacheAPIListener : OrchardApiEndListener
{
                              ...
                              endListenerParameters: Any?)
    {
        ...
        if (remoteRequest.method == RemoteRequest.Method.POST)
        {
            ...
            {
                if (body.get(Constants.REQUEST_CONTENT_TYPE).asString == context.getString(R.string.orchard_citizen_user_reports_content_type))
                {
                    val realm = Realm.getDefaultInstance()
                    realm.beginTransaction()
                    val cache = RequestCache.findCacheWith(context.getString(R.string.orchard_user_reports_display_alias))
